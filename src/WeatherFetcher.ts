...
export async function WeatherFetcher(city : string, infoTemp : boolean) {
    ...
    let newWeather = {weather: data.weather[0].main, temperature: kelToCF(data.main.temp, infoTemp), wind: wind, name: cityData[0].name, url: await loadImage(data.weather[0].main), deg: windDeg}
    return newWeather
}
